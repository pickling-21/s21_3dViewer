...
//    char file[50] = {"skull.obj"};
    ...
    all_surfaces = surface_formation(file, &count_surfaces, all_surfaces, all_points);
...
//            printf("v%ld: %1.6f %1.6f %1.6f\n", all_surfaces[i].one_point[j] - all_points + 1, all_surfaces[i].one_point[j]->x, all_surfaces[i].one_point[j]->y, all_surfaces[i].one_point[j]->z);
//
//        }
//        printf("|====================|\n");
//    }
...
void figure_rotation(int top_pointers, Point *all_points, double degree_x, double degree_y, double degree_z) {
    ...
    for (int i = 0; i <= top_pointers; ++i) {
        ...
        temp_x = cos_z * temp_x - sin_z * temp_y;
        temp_y = sin_z * temp_x + cos_z * temp_y;
        all_points[i].x= temp_x;
        all_points[i].y = temp_y;
...
void figure_centering(int top_pointers, ExtremeValues *extreme_values, Point *all_points) {
    double center_x, center_y, center_z;
    double dmax, coefficient;
    center_x = extreme_values->x_min + ((extreme_values->x_max - extreme_values->x_min) / 2);
    center_y = extreme_values->y_min + ((extreme_values->y_max - extreme_values->y_min) / 2);
    center_z = extreme_values->z_min + ((extreme_values->z_max - extreme_values->z_min) / 2);
    dmax = delta_max(extreme_values->x_max - extreme_values->x_min, extreme_values->y_max - extreme_values->y_min, extreme_values->z_max - extreme_values->z_min);
...
Point *point_reading(char *file, int *top_pointers, ExtremeValues *extreme_values) {
    FILE *f = NULL;
    int size_all_points = SIZE;
    Point *all_points = (Point *)malloc(size_all_points * sizeof(Point));
    if (((f = fopen(file, "r")) != NULL) && (all_points != NULL)) {
        all_points = point_processing(f, top_pointers, &size_all_points, extreme_values, all_points);
    }
    fclose(f);
    return all_points;
}

Point *point_processing(FILE *f, int *top_pointers, int *size_all_points, ExtremeValues *extreme_values, Point *all_points) {
    ...
    while ((c = fgetc(f)) != EOF) {
        if ((c == ' ') && (past_char == 'v')) {
            if ((*top_pointers) == (*size_all_points)) {
                ...
                all_points = (Point *)realloc(all_points, (*size_all_points) * sizeof(Point));
            }
            fgets(buffer_str, 99, f);
            int count_coord = 0, exp = 0;
            int flag_result = 0, flag_dot = 0, flag_mines = 0, flag_mines_exp = 0, flag_exp = 0;
            double result = 0;
            for (int i = 0; count_coord < 3; ++i) {
                if ((buffer_str[i] != ' ') && (buffer_str[i] != '\n') && (buffer_str[i] != '\0') && (buffer_str[i] != 13)) {
                    ...
                    finding_extreme_values(result, count_coord, *top_pointers, extreme_values);
                    flag_mines = flag_result = flag_dot = flag_mines_exp = 0;
                    coordinate_recording_counter(&count_coord, *top_pointers, &result, all_points);
                    if (count_coord == 3) {
                        (*top_pointers)++;
                    }
                }
...
void point_attribute_correction(int flag_mines, int flag_mines_exp, int *exp, double *result) {
    if (flag_mines == 1) {
        (*result) *= -1;
    }
    if ((*exp) != 0) {
...
void finding_extreme_values(double result, int count_coord, int top_pointers, ExtremeValues *extreme_values) {
    if (top_pointers == 0) {
        switch (count_coord) {
            case 0:
        ...
        switch (count_coord) {
            case 0:
                if (result > extreme_values->x_max)
                    extreme_values->x_max = result;
                if (result < extreme_values->x_min)
                    extreme_values->x_min = result;
                break;
            case 1:
                if (result > extreme_values->y_max)
                    extreme_values->y_max = result;
                if (result < extreme_values->y_min)
                    extreme_values->y_min = result;
                break;
            case 2:
                if (result > extreme_values->z_max)
                    extreme_values->z_max = result;
                if (result < extreme_values->z_min)
                    extreme_values->z_min = result;
                break;
        }
    }
...
void coordinate_recording_counter(int *count_coord, int top_pointers, double *result, Point *all_points) {
    switch (*count_coord) {
        case 0:
...
Surface *surface_formation(char *file, int *count_surfaces, Surface *all_surfaces, Point *all_points) {
    FILE *f;
        ...
        while((c = fgetc(f)) != EOF) {
            ...
            if ((c == ' ') && (past_char == 'f')) {
                if ((*count_surfaces) == size_all_surfaces) {
                    ...
                    all_surfaces = (Surface*)realloc(all_surfaces, size_all_surfaces * sizeof(Surface));
                }
                fgets(buffer_str, 999, f);
                points_in_space(buffer_str, (*count_surfaces), top_pointers_now - 1, all_surfaces, all_points);
                (*count_surfaces)++;
            }
            past_char = c;
        }
...
void points_in_space(char *buffer_str, int count_surfaces, int top_pointers, Surface *all_surfaces, Point *all_points) {
    int res = 0;
    ...
    for (int i = 0; buffer_str[i] != 0; ++i) {
        if ((buffer_str[i] == ' ') && (((buffer_str[i + 1] >= '0') && (buffer_str[i + 1] <= '9')) || (buffer_str[i + 1] == '-')))
            number_of_points++;
    }
    all_surfaces[count_surfaces].one_point = (Point**)malloc(number_of_points * sizeof(Point*));
    for (int i = 0; count_point_in_surface < number_of_points; ++i) {
        if ((buffer_str[i] != ' ') && (buffer_str[i] != '\n') && (buffer_str[i] != '\0') && (buffer_str[i] != 13)) {
            ...
            all_surfaces[count_surfaces].one_point[count_point_in_surface] = all_points + res;
